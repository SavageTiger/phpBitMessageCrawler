...
class Protocol
{
    ...
    public function generateVersionPackage($remoteIP, $remotePort, $localPort)
    {
        ...
        $payload .= $this->pack_double(1, 64, false); // Bitflags
        $payload .= $this->pack_double(time(), 64, false); // Bitflags
        
        // Remote IP
        $payload .= $this->pack_double(1, 64, false); // Remote bitflags (what a odd this to send...)
        $payload .= hex2bin('00000000000000000000FFFF');
        $payload .= pack('N', ip2long($remoteIP));
        $payload .= pack('n', $remotePort);
        
        // Local IP
        $payload .= $this->pack_double(1, 64, false); // Bitflags again?
        $payload .= hex2bin('00000000000000000000FFFF');
        $payload .= pack('N', ip2long('127.0.0.1')); // Ignored by remote client anyway...
        $payload .= pack('n', $localPort);

        $payload .= $this->pack_double(rand(919191, 2929292992), 64, false); // Random

        // User agent
        $agent = 'phpBitMessageCrawler/0.1';
        $payload .= $this->varInt(strlen($agent));
        $payload .= $agent;
        
        // Stream number
        $payload .= $this->varInt(1);
        $payload .= $this->varInt($streamNumber);
    ...
    public function recievePackage($data, $socket)
    {
        ...
        // Big package, download the rest otherwise the hash check is will fail
        if ($size > 1024) {
    ...
    protected function checkRemoteVersion($payload, $socket)
    {
        ...
        if ($version >= 2) {
            ...
            $stringSize = $this->decodeVarInt(substr($payload, 80, 84));           
    ...
    protected function isVerAck($data) {
        $data = bin2hex($data);

        return (substr($data, 0, 20) === 'e9beb4d976657261636b');
    }

    protected function decodeVarInt($input)
    {
        // TODO
    ...
    protected function varInt($input)
    {
        // TODO        
        return pack('C', $input);
    }

    protected function pack_double($in, $pad_to_bits = 64, $little_endian = true)
    {
        $in = decbin($in);
